...
plt.title('Monthly Pageviews 2022',font='Montserrat',weight='bold',fontsize=24,loc='left')
...
def annotate():
	change_percent = ((df['sum_view_count'].iat[-1] - df['sum_view_count'].iat[-2]) /  df['sum_view_count'].iat[-2]) * 100
	if change_percent > 0:
		annotation = f" +{change_percent:.1f}% MoM" + " " + note
	else:
		annotation = f" {change_percent:.1f}% MoM" + " " + note
	plt.annotate(annotation,
		xy = (df['timestamp'].iat[-1],df['sum_view_count'].iat[-1]),
		xytext = (20,-5),
		xycoords = 'data',
...
#data notes
plt.figtext(0.1, 0.025, "Graph Notes: Created by Hua Xi 12/12/22 using data from https://docs.google.com/spreadsheets/d/1Aw5kjj47cEi-PSX0eApCUp3Ww9_XNyARxcdoL9QnHp4", fontsize=8, family='Montserrat',color= wmf_colors['black25'])
