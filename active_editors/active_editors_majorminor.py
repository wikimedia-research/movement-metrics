...
#for highlighting
yoy_highlight = pd.concat([df.iloc[-12,:],df.iloc[-1,:]],axis=1).T
...
current_yvalues = ax.get_yticks()
print(type(current_yvalues[0]))
#plt.gca().set_yticklabels([y_label_formatter(x) for x in current_values])
ax.set_yticklabels([y_label_formatter(x) for x in current_yvalues],fontname = 'Montserrat',fontsize=14)
...
#month labels
months = mdates.MonthLocator(bymonth=month_interest)
months_fmt = mdates.DateFormatter('%b')
ax.xaxis.set_minor_locator(months)
ax.xaxis.set_minor_formatter(months_fmt)
for tick in ax.xaxis.get_minorticklabels():
    tick.set_fontfamily('Montserrat')
ax.xaxis.set_tick_params(which='minor',size=12)
ax.tick_params(axis="x",which="minor",length=5,pad=5)
#
